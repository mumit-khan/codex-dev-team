...
| 13 | **B-13 [DONE]** Programmatic stoplist enforcement on lighter tracks | Ported `scripts/stoplist.js` from claude with the regex set unchanged. Wired into `runTrack` to refuse `/quick`/`/nano`/`/config-only`/`/dep-update` on match; `--force` bypasses. 27 tests cover the regex set, gather flow, and CLI integration. |
| 14 | **B-10 [DONE]** Extract `tests/_framework-contract.js` shared module | RULES (7 items, codex-claude-shared) + CODEX_ONLY_RULES (2: execution-profiles, roles) + SKILLS + ROLES + ADAPTERS + STAGE_NUMBERS + STAGE_SCHEMAS. Three test files (contract / release / parity-check) now import from the module. |
| 15 | **B-21 [DONE]** Split `.codex/rules/pipeline.md` into core / build / tracks | Three new sub-files; pipeline.md becomes a thin index. Stoplist content moved to pipeline-tracks.md; parity-check.js scan target updated; orchestrator.md startup updated to read all three; release.test.js + parity-check.test.js fixtures patched. 258/258 tests still pass. |
