...
|---|---|---|
| 1 | **B-4** schema `description` fields | Direct copy from claude's schemas; zero behavioural impact. Improves contributor UX immediately. |
| 2 | **B-5** `templates/README.md` | One-line-per-template doc; copy claude's verbatim with the `.claude` → `.codex` mention swap. |
