...
const SCRIPT = path.join(ROOT, "scripts", "codex-team.js");
...
describe("stoplist patterns", () => {
  const cases = [
    ...
    ["fix production checkout timeout", false],
  ...
  for (const [input, shouldMatch, category] of cases) {
    it(`${shouldMatch ? "matches" : "ignores"}: "${input}"`, () => {
      ...
      if (shouldMatch) {
        ...
        assert.equal(matches.length, 0, `expected ${input} not to match`);
      }
    });
  }
...
describe("stoplist gather", () => {
  it("returns empty matches when description is benign and cwd has no git", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "codex-stoplist-"));
    try {
      const matches = checkStoplist({ description: "fix typo in README", cwd: tmp });
  ...
  it("matches the description even when cwd has no git history", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "codex-stoplist-"));
    try {
      const matches = checkStoplist({ description: "add password reset flow", cwd: tmp });
...
describe("codex-team CLI: stoplist enforcement", () => {
  ...
  it("/quick rejects a stoplist-matching description", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "codex-stoplist-cli-"));
    try {
  ...
  it("/hotfix is exempt from the stoplist", () => {
    ...
    try {
      fs.writeFileSync(path.join(tmp, "package.json"), JSON.stringify({ scripts: {} }));
      const result = runCLI(["hotfix", "Login endpoint returning 500 on valid creds"], tmp);
      assert.notEqual(result.status, 2);
      assert.doesNotMatch(result.stderr, /safety stoplist/i);
    } finally {
      fs.rmSync(tmp, { recursive: true, force: true });
